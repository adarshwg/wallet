...
class TestTransactionManager(unittest.TestCase):
    ...
    @patch('transaction_manager.db_operations.get_current_month_transactions')
    def test_get_current_month_transactions(self, mock_get_current_month_transactions):
        mock_get_current_month_transactions.return_value = [
            {'id': 1, 'amount': 100}
        ]
        result = TransactionManager.get_current_month_transactions('Alice')
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]['amount'], 100)
        mock_get_current_month_transactions.assert_called_once_with('Alice')
